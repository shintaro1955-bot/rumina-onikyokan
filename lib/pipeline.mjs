...
const OBJECTION_DICT = [
  // 引き際の判定に効くので、強い拒絶と「訪問販売お断り」を先に当てる（順番を変えると分類が変わる）。
  { label: '断固拒否', re: /(帰って|かえって|二度と|来ないで|こないで|通報|警察|訴え|しつこい|迷惑)/ },
  { label: '訪問販売お断り', re: /(訪問販売|飛び込み|セールス|勧誘(は)?(お断り|結構)|営業(は)?(お断り|結構))/ },
  { label: '断固拒否', re: /(お断り|おことわり)/ },
...
// 断りの後の一言が「引き下がっただけ」か。粘る意思のある語が無ければ退却とみなす。
const RETREAT_RE = /(失礼しま|また来ま|また改めて|また今度|すみませんでした|ありがとうございました)/;
const PUSH_RE = /(だけ|ちなみに|一度|それでも|何とか|お願い|見せて|確認して|いかがですか|ませんか|30秒)/;
const isRetreat = (t) => { const x = String(t || ''); return RETREAT_RE.test(x) && !PUSH_RE.test(x); };
...
/* ---- トーク再現率：成功モデルの勝ちの型を、各人がどれだけ写せているか ---- */
// モデルの型と、会話が成立した訪問での“使用率の目安”。
// ★これは初期値。成功モデル本人の録音を「成功モデル登録」すると
//   computeModelProfile() が実録音から rate を算出し、この基準を置き換える。
export const MODEL_TALK = [
  { key: '冒頭フック', rate: 95, tip: '一言目で「電気の健康診断／明細／電気代」を出す' },
  { key: '明細ドライブ', rate: 85, tip: '「明細だけ30秒」で警戒を下げて数字に入る' },
  { key: '問題提起', rate: 80, tip: '「電気代上がってませんか」を相手に言わせる' },
  { key: '創蓄・補助金', rate: 70, tip: 'エコキュートを入口に創蓄・補助金の“今だけ”を出す' },
  { key: '切り返し', rate: 75, tip: '断りに最低1回は切り返して会話へ戻す' },
  { key: '2択クロージング', rate: 75, tip: '最後は“AかB”の二択で日程を置いてくる' },
];
// クロージングは2択クロージングの部分点として扱う
function movesOf(p) { return new Set(winMoves(p).map(x => x.key === 'クロージング' ? '2択クロージング' : x.key)); }
function talkFidelity(pings, modelTalk) {
  const model = modelTalk || MODEL_TALK;
  const engaged = pings.filter(p => p.customerSpoke);   // 型を出せる＝会話が成立した訪問
  const denom = engaged.length;
  const used = engaged.map(movesOf);
  const moves = model.map(m => {
    const cnt = denom ? used.filter(s => s.has(m.key)).length : 0;
    const repRate = denom ? Math.round(cnt / denom * 100) : 0;
    return { key: m.key, tip: m.tip, repRate, modelRate: m.rate, gap: repRate - m.rate, fidelity: m.rate ? Math.min(100, Math.round(repRate / m.rate * 100)) : 100 };
  });
  const overall = moves.length ? Math.round(moves.reduce((s, x) => s + x.fidelity, 0) / moves.length) : 0;
  const weakest = moves.slice().sort((a, b) => a.gap - b.gap)[0] || null;
  return { overall, moves, denom, weakest };
}

/* ---- 成功モデル登録：モデル本人の録音の pings から“基準値”を実算出 ---- */
// 各型を、会話が成立した訪問で実際に何割使っていたかを rate として採用。
// tip（処方文）は初期MODEL_TALKから引き継ぐ。denom はモデル録音の会話訪問数。
export function computeModelProfile(pings, base = MODEL_TALK) {
  const engaged = pings.filter(p => p.customerSpoke);
  const denom = engaged.length;
  const used = engaged.map(movesOf);
  const profile = base.map(m => {
    const cnt = denom ? used.filter(s => s.has(m.key)).length : 0;
    const rate = denom ? Math.round(cnt / denom * 100) : m.rate;
    return { key: m.key, rate, tip: m.tip };
  });
...
export function analyze(segments, ctx) {
  ...
  const pings = clusters.map((c, i) => {
    ...
    // 切り返し：お客様の断りターンの直後に営業が"粘った"回数（分離あり＝実カウント）。
    // ※「失礼しました」で下がっただけの一言は切り返しに数えない。数えると
    //   「断られても粘ったか」の指標が、引き下がった訪問まで拾って意味を失う。
    let rebuttals = 0;
    if (objection && diarized) {
      for (let k = 0; k < c.segs.length - 1; k++) {
        if (c.segs[k].speaker === 'customer' && objection.re.test(c.segs[k].text)
          && c.segs[k + 1].speaker === 'sales' && !isRetreat(c.segs[k + 1].text)) rebuttals++;
      }
    } else if (objection) {
  ...
  // 総ピンポン数（不在含む真の分母）の決定。優先度：cyzen確定 > GPS > 文面
  // cyzenの「本日の訪問件数（不在も含む）」＝鬼教官の総ピンポン数と同義なので最優先で採用。
  const cyzenPings = Number.isFinite(ctx.confirmedPings) && ctx.confirmedPings > 0 ? ctx.confirmedPings : null;
  const pingCountFromGps = !cyzenPings && !!(gps && gps.totalStops >= totalPings);
  const pingSource = cyzenPings ? 'cyzen' : pingCountFromGps ? 'gps' : 'transcript';
  const denom = cyzenPings || (pingCountFromGps ? gps.totalStops : totalPings);

  // 確定アポ数：CRM/cyzenから渡されれば自動採用（文面推定を上書き）
  ...
  const analysis = {
    ...
    objectionRanking,
    hourly,
    // cyzen照合（量の確定）。ctx.cyzen が渡されたときのみ付く。
    cyzen: ctx.cyzen || null,
    winTalk: winTalkAnalysis(pings),
    talkFidelity: talkFidelity(pings, ctx.modelTalk),
    ...
    quality: (() => {
      ...
      const est = [];
      if (pingSource === 'transcript') est.push('homeResponseRate', 'conversationRate');
      ...
      notes.push(diarMethod === 'acoustic' ? '話者分離：音響（確定）。'
        ...
        : '話者分離：未接続。会話系KPIは推定。');
      notes.push(pingSource === 'cyzen' ? `総ピンポン数：cyzenの「本日の訪問件数（不在も含む）」から確定（${denom}件）。`
        : pingSource === 'gps' ? `総ピンポン数：GPS停止クラスタから自動算出（${gps.totalStops}件＝訪問${gps.visitClusters}+不在${gps.noAnswerStops}）。`
        : '総ピンポン数：会話数ベース。GPS/cyzenで確定要。');
      notes.push(apoConfirmed ? 'アポ数：cyzen/CRMから自動取得（確定）。' : 'アポ率：文面判定＝cyzen/結果入力で確定。');
      notes.push(gps ? 'GPS照合済：空白は移動/接客/実サボりに裏取り済み。' : 'GPS未接続：サボりは要裏取り。');
      return {
        engine: 'whisper',
        speakerSeparation: diarMethod,
        gps: gps ? 'connected' : 'none',
        pingCountSource: pingSource,
