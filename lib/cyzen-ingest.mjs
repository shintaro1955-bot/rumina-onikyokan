...
export async function ingestReports({ days = 31 } = {}) {
  ...
  let mapped = 0;
  const diagKeys = reports[0] ? Object.keys(reports[0]).slice(0, 16) : [];
  for (const r of reports) {
    ...
    if (name.includes('勤務終了')) { const v = visitCountOf(items); if (v != null) d.visitsSelf = Math.max(d.visitsSelf, v); d.endReport = true; d.workEnd = pick(r, 'report_datetime', 'created_at') || d.workEnd; }
  ...
  const diag = { users: users.size, reports: reports.length, mapped, days: day.size, reportKeys: diagKeys, sampleDay: [...day.values()][0] || null };
