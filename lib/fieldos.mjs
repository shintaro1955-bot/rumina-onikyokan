...
function root() {
  const db = getDb();
  if (!db.fieldos) db.fieldos = {};
  const f = db.fieldos;
  f.goals ||= {}; f.learning ||= {}; f.posts ||= []; f.reactions ||= []; f.reads ||= {};
  f.comments ||= []; f.notifs ||= {}; f.notifSettings ||= {}; f.events ||= []; f.insights ||= {};
  return f;
}
/* AIコーチの1日1回キャッシュ（Claude呼び出しを本人×日で1回に抑える） */
export function getInsight(user) { return root().insights[user] || null; }
export function setInsight(user, coach) { const f = root(); f.insights[user] = { day: new Date().toISOString().slice(0, 10), coach }; save(); return coach; }
const genId = pfx => pfx + Date.now().toString(36) + Math.floor(Math.random() * 1e6).toString(36);
...
export function setGoal(user, g) {
  const prev = getGoal(user) || {};
  const rec = {
    visits: Math.max(1, Math.round(g.visits != null ? g.visits : (prev.visits || 50))),
    apo: g.apo !== undefined ? (g.apo == null || g.apo === '' ? null : Math.max(0, Math.round(g.apo))) : (prev.apo ?? null),
    why: g.why !== undefined ? String(g.why || '').slice(0, 600) : (prev.why || ''),
    updatedAt: now(),
  };
...
export function momentum(agg, user, learnRate = 0, standard = null) {
  const w = getWeights();
  if (!agg || agg.empty) return { score: null, confidence: 'insufficient', parts: null, weights: w, note: '算出に十分なデータがありません' };
  // 基準は会社共通（トップの1日）。個人の目標設定は廃止したので、渡されなければ従来の既定値に戻す。
  // ここを既定値50のままにすると、トップが18.4件/日なのに全員が50件で採点されて誰も届かない。
  const target = (standard && standard.visits > 0) ? standard.visits : goalVisits(user);
  const parts = {
    goal: clamp01((agg.visitsPerDay || 0) / target),                         // 行動目標達成率
    conversion: clamp01((agg.apoRate || 0) / ((standard && standard.apoRate > 0) ? standard.apoRate : 5.5)),  // アポ転換（基準＝トップのアポ率）
...
export function addPost(post) {
  ...
  const rec = {
    ...
    author: post.author || '本部', authorUser: post.authorUser || null, important: !!post.important, scope: post.scope || 'company',
    createdAt: now(),
  };
...
export function postAuthorUser(postId) { const p = root().posts.find(x => x.id === postId); return p ? p.authorUser || null : null; }

/* ---------- コメント（投稿・フィードへのスレッド） ---------- */
export function addComment(user, name, targetId, text) {
  const f = root();
  const rec = { id: genId('c'), targetId: String(targetId), user, name: name || user, text: String(text || '').slice(0, 1000), at: now() };
  if (!rec.text.trim()) return null;
  f.comments.push(rec); f.comments = f.comments.slice(-3000); save();
  return rec;
}
export function listComments(targetId) { return root().comments.filter(c => c.targetId === String(targetId)).sort((a, b) => a.at.localeCompare(b.at)); }
export function commentCount(targetId) { return root().comments.filter(c => c.targetId === String(targetId)).length; }
export function deleteComment(id, user, isOwner) {
  const f = root(); const c = f.comments.find(x => x.id === id); if (!c) return false;
  if (!isOwner && c.user !== user) return false;
  f.comments = f.comments.filter(x => x.id !== id); save(); return true;
}

/* ---------- 通知（乱発しない：日次タイプは1日1回・イベントは都度） ---------- */
export function pushNotif(user, n, dedupKey) {
  if (!user) return;
  const f = root(); f.notifs[user] ||= [];
  if (dedupKey) { const k = dedupKey + ':' + dayKey(); if (f.notifs[user].some(x => x.dk === k)) return; n.dk = k; }
  f.notifs[user].unshift({ id: genId('n'), type: n.type || 'info', title: n.title || '', body: n.body || '', link: n.link || null, at: now(), read: false, dk: n.dk });
  f.notifs[user] = f.notifs[user].slice(0, 100); save();
}
export function pushNotifAll(users, n, exceptUser) {
  for (const u of users) if (u !== exceptUser) pushNotif(u, n, n.type);
}
export function listNotifs(user) { return (root().notifs[user] || []).slice(0, 40); }
export function unreadCount(user) { return (root().notifs[user] || []).filter(n => !n.read).length; }
export function markNotifsRead(user) { const f = root(); (f.notifs[user] || []).forEach(n => n.read = true); save(); }
export function getNotifSettings(user) { return root().notifSettings[user] || { morning: true, pace: true, praise: true, review: true }; }
export function setNotifSettings(user, s) { root().notifSettings[user] = { ...getNotifSettings(user), ...s }; save(); return getNotifSettings(user); }

/* ---------- 分析イベント（GPS座標は送らない） ---------- */
export function track(user, event, props) {
  const f = root();
  f.events.push({ user: user || null, event: String(event || '').slice(0, 40), props: props || null, at: now() });
  f.events = f.events.slice(-5000); save();
}
