...
export function todayGap({ name = '', code = '', date = '' } = {}) {
  ...
  // あるべき姿は「一番成績がいい人の1日」。上位2%は量の目安として脇に置く。
  // ※上位2%の訪問(56件/日)は量で押す型の数字で、トップのアポ率とは別の型。
  //   2つ並べると目標が2つになるので、追いかける先は1人に絞る。
  // 方針（2026-09-25 オーナー決定）：あるべき姿は「一番アポを取っている人の1日」。
  // 量ではなく決める率で追う（トップは訪問18.4件＝中央値並みで、アポ率7.6%で1位）。
  // 歩いた距離は**目標を置かない**。GPS打刻が粗く中央値0.1km/日と実態を表しておらず、
  // 目標にすると全員が未達になるだけ。実測とみんなの平均だけ見せる。
  const best = topPerformer(30);
  const goal = best
    ? { visits: best.visitsPerDay, apo: best.apoPerDay, apoRate: best.apoRate, walkKm: null }
    : { visits: top.visits, apo: top.apo, apoRate: null, walkKm: null };
  const gapTop = shortTo(goal);
  return {
    ready: true, date: day, found: !!me, people: actives.length,
    best, goal,               // goal=追いかける先の1日（実名の人の数字）
    ...
    focus: me ? focusOf(me, goal, best) : null,
  };
}

/* 乖離のうち、いま手を入れるべき一段を決める（下から順に見る）。
   行動量が足りない人にトークを教えても効かない。 */
function focusOf(me, goal, best) {
  const who = best ? `${best.name}さん` : 'トップ';
  if (goal.visits != null && me.visits < goal.visits * 0.6) {
    return { rung: '行動量', why: `${who}は1日${goal.visits}件。その6割にも届いていません。まず母数を作る。`, action: 'field' };
  }
  if (me.visits > 0 && me.apo === 0) {
    return { rung: 'トーク', why: `訪問はしているのにアポが0件。${best && best.apoRate != null ? `${who}は同じ訪問からアポ率${best.apoRate}%を出しています。` : ''}玄関先の型が崩れている可能性が高い。`, action: 'roleplay' };
  }
  if (goal.visits != null && me.visits < goal.visits) {
    return { rung: '行動量', why: `${who}は1日${goal.visits}件。あと${+(goal.visits - me.visits).toFixed(1)}件で同じ土俵に立てます。`, action: 'field' };
  }
  if (goal.apo != null && me.apo < goal.apo) {
    return { rung: 'トーク', why: `訪問数は足りています。${who}はこの訪問数からアポ${goal.apo}件を作ります。差はトークの型。`, action: 'roleplay' };
  }
  return { rung: '維持', why: `訪問もアポも${who}の水準にあります。この型を崩さないこと。`, action: 'roleplay' };
}
