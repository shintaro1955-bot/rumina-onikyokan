...
export function buildFacts() {
  ...
  /* 歩行（GPS打刻から自動算出。報告書と違い記録漏れが起きにくい） */
  let walking = null;
  if (walk.ready()) {
    const w = walk.stats({ days: WINDOW_DAYS });
    if (w.ready && w.rows.length) {
      walking = {
        count: w.count, totalKm: w.totalWalkKm, medianPerDay: w.medianWalkPerDay,
        top3: w.rows.slice(0, 3).map(r => ({ name: r.name, km: r.walkKm, perDay: r.walkPerDay })),
      };
    }
  }
  ...
  return {
    ...
    ranked: byVpd.map((r, i) => ({ rank: i + 1, name: r.name, code: r.code, vpd: r.vpd, visits: r.visits, days: r.days, apo: r.apo })),
...
export function fallbackMessage(f) {
  ...
  L.push('');
  if (f.walking) {
    L.push('■ 今月よく歩いた人（GPS計測）');
    f.walking.top3.forEach((w, i) => L.push(`${i + 1}. ${w.name} ${w.km}km（1日${w.perDay}km）`));
    L.push(`　全体 ${f.walking.totalKm.toLocaleString()}km ・ 1日の中央値 ${f.walking.medianPerDay}km`);
    L.push('');
  }
...
const SYSTEM = [
  ...
  '称えるのは「伸びた人」「訪問トップ3」「歩行トップ3」だけ。下位を晒すことは会社の規則で禁止されている。',
...
export async function buildMessage() {
  ...
  const safe = {
    ...
    belowCount: f.belowCount, totalVisits: f.totalVisits, walking: f.walking,
  };
  ...
  const user = [
    ...
    '・実名を出すのは「今週伸びた人」「訪問数トップ3」「よく歩いた人トップ3」だけ。',
    ...
    '3) 訪問数トップ3',
    '4) 今月よく歩いた人トップ3（GPS計測。歩行距離は自動記録なので信頼できる数字）',
    '5) 基準未達の人数のみ',
    '6) 明日の行動を促す締めを一言',
    '',
