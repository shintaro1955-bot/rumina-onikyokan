...
function buildQuery(opts, drop = new Set()) {
  ...
  // ロープレの一言など単一話者・低遅延が要る用途は opts.diarize=false で丸ごと外す（速く・短文の精度も上がる）。
  if (opts.diarize !== false) {
    if (drop.has('diarize_model')) p.set('diarize', 'true');
    else p.set('diarize_model', 'latest');
  }
  if (opts.utterances !== false && !drop.has('utterances')) p.set('utterances', 'true');  // 発話単位でまとめて返す
