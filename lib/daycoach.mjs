/* ============================================================
   ...
   ============================================================ */
import { scanPings } from './talkrisk.mjs';
...
export function buildDayFacts(reports = [], { name = null, date = null } = {}) {
  ...
  return {
    ...
    risks: scanPings(pings),   // 言ってはいけない言い回しの検出（決定論・要確認まで）
...
const SYSTEM = [
  ...
  '',
  '【危ない言い回し】検出が渡された場合は、成績の話より先に必ず取り上げる。「重い」は法令に触れる恐れがあるので、言い換えを具体的に示して必ず直させる。',
  'ただし「違反です」と断定しない。検出は辞書によるもので、最終判断は人がする。「この言い方は危ないので変える」という書き方にする。',
...
const SCHEMA = {
  ...
  properties: {
    ...
    risky: { type: 'array', items: { type: 'string' }, description: '危ない言い回しについて、今すぐ直す指示（検出が無ければ空配列）。「その言い方はやめて、こう言う」の形で' },
  ...
  required: ['summary', 'mistakes', 'fixes', 'risky', 'tomorrow', 'keep'],
...
export async function buildDayCoach(facts) {
  ...
  const lines = [
    ...
    '',
    '【危ない言い回しの検出（辞書による検出＝要確認。違反と断定しない）】',
    ...((f.risks && f.risks.items.length)
      ? f.risks.items.map(r => `[${r.level === 'high' ? '重い' : '要注意'}] ${r.label}（${r.law}）${r.count}回 例：「${r.quotes[0]}」`)
      : ['（検出なし）']),
    '',
    '【抜粋（実際のトーク）】',
