...
/** ある日の担当者ごとの歩行距離(km)。トップ画面の「今日どれだけ歩いたか」用。 */
export function kmByCodeOn(date) {
  const out = new Map();
  const d = db(); if (!d || !date) return out;
  for (const r of d.day.values()) {
    if (r.date !== date) continue;
    out.set(r.code, +(r.walkM / 1000).toFixed(1));
  }
  return out;
}
