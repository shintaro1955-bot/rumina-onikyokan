/* ============================================================
   FF寺子屋 — 対象者への個別案内（本人へのDM）
   対象者リストは人事・KPI側で確定済み（基準期間=7月の週平均 vs 直近週を比較し、
   役割に応じた指標＝アポインター:アポ獲得数／クローザー:クロ成約数 が
   低下したまま戻っていない、かつcyzen上で稼働継続している人）。
   ここでは**選定はしない**。渡されたCSVを読み、本人への文面を作るだけ。

   方針（_RULE.md / SKILL.md）：
     ・本人へのDMのみ。グループに実名を出さない。
     ・下がった指標は事実として具体的に示すが、**上がっている指標は必ず認める**
       （「実態と乖離がある」と言われないため。人格ではなく数字と行動の話に留める）
     ・出退勤の記録が無い人は自動送信せず「要確認」に回す（休職・離脱の可能性）
   ============================================================ */
import { readFileSync, existsSync } from 'node:fs';
import { join } from 'node:path';
import { DATA_DIR } from './store.mjs';

const LIST = process.env.TERAKOYA_LIST || join(DATA_DIR, 'terakoya-targets.csv');
const CONF = join(DATA_DIR, 'terakoya.json');
// 運用で差し替えたい場合は DATA_DIR/terakoya.json を置く。無ければ同梱の既定値を使う。
const CONF_DEFAULT = new URL('../config/terakoya.default.json', import.meta.url).pathname;
const WD = ['日', '月', '火', '水', '木', '金', '土'];
const normName = s => String(s || '').replace(/[\s　]+/g, '').normalize('NFKC');
const num = v => { const n = parseFloat(String(v ?? '').trim()); return Number.isFinite(n) ? n : 0; };

function parseCsv(text) {
  const rows = []; let f = '', row = [], q = false;
  const t = text.replace(/^﻿/, '');
  for (let i = 0; i < t.length; i++) {
    const c = t[i];
    if (q) { if (c === '"') { if (t[i + 1] === '"') { f += '"'; i++; } else q = false; } else f += c; }
    else if (c === '"') q = true;
    else if (c === ',') { row.push(f); f = ''; }
    else if (c === '\n') { row.push(f); rows.push(row); row = []; f = ''; }
    else if (c !== '\r') f += c;
  }
  if (f || row.length) { row.push(f); rows.push(row); }
  const head = rows.shift() || [];
  return rows.filter(r => r.some(x => (x || '').trim()))
    .map(r => Object.fromEntries(head.map((h, i) => [h.trim(), (r[i] || '').trim()])));
}

/** 寺子屋の開催情報（日程が無ければ null＝案内文を出さない安全弁）。 */
export function schedule() {
  try {
    const path = existsSync(CONF) ? CONF : (existsSync(CONF_DEFAULT) ? CONF_DEFAULT : null);
    if (!path) return null;
    const t = JSON.parse(readFileSync(path, 'utf8'));
    const ss = (t.sessions || []).filter(s => s && s.date);
    if (!ss.length) return null;
    const today = new Date().toISOString().slice(0, 10);
    const next = ss.filter(s => s.date >= today).sort((a, b) => a.date.localeCompare(b.date))[0]
      || ss.slice().sort((a, b) => b.date.localeCompare(a.date))[0];
    return { ...t, next };
  } catch (e) { console.warn('[terakoya] 設定読込失敗:', e.message); return null; }
}
const fmtDate = ymd => {
  const d = new Date(ymd + 'T00:00:00Z');   // 日付のみを扱うのでUTC固定（曜日ズレ防止）
  return `${+ymd.slice(5, 7)}/${+ymd.slice(8, 10)}(${WD[d.getUTCDay()]})`;
};

export const ready = () => existsSync(LIST);

/**
 * 対象者ごとの本人向け文面を作る。
 * @returns {{ok:boolean, count:number, needsReview:number, targets:Array}}
 */
export function buildInvites() {
  if (!existsSync(LIST)) return { ok: false, error: `対象者リストがありません（${LIST}）` };
  const rows = parseCsv(readFileSync(LIST, 'utf8')).filter(r => r['氏名']);
  if (!rows.length) return { ok: false, error: '対象者リストが空です' };
  const T = schedule();

  const targets = rows.map(r => {
    const name = r['氏名'], cause = r['要因分類'] || '', role = r['役割区分'] || '';
    const apoB = num(r['基準週平均アポ獲得数']), apoN = num(r['直近週アポ獲得数']);
    const kB = num(r['基準週平均成約数(クロ側)']), kN = num(r['直近週成約数(クロ側)']);
    const workDays = num(r['8月出退勤日数']);
    const lastIn = r['最終出勤日時'] || '';

    // 出退勤の記録が無い＝休職・離脱の可能性。自動送信せず人が確認する。
    const needsReview = workDays <= 0;
    // 明示的な送信除外（人の判断で外した人。自動判定より優先する）
    const excludeList = ((T && T.excludeNames) || []).map(normName);
    const excluded = excludeList.includes(normName(name));

    // 下がった指標（要因分類に対応）と、上がっている指標
    const isApo = /アポ低下/.test(cause);
    const down = isApo
      ? { label: 'アポ獲得数', base: apoB, now: apoN }
      : { label: 'クロージング成約数', base: kB, now: kN };
    const other = isApo
      ? { label: 'クロージング成約数', base: kB, now: kN }
      : { label: 'アポ獲得数', base: apoB, now: apoN };
    const improved = other.now > other.base;
    const drop = +(down.base - down.now).toFixed(1);

    const L = [];
    L.push(`${name} さん`);
    L.push('');
    L.push('Ruminaです。7月と直近週の数字を見ています。');
    L.push('');
    L.push('■ 落ちたまま戻っていない指標');
    L.push(`　${down.label}：7月の週平均 ${down.base} → 直近週(8/5〜8/9) ${down.now}`);
    if (drop > 0) L.push(`　週あたり ${drop} 件ぶん落ちたままです。`);
    if (improved) {
      L.push('');
      L.push('■ 上がっているところ');
      L.push(`　${other.label}：${other.base} → ${other.now}。ここは伸びています。`);
      L.push('　だからこそ、落ちた方を戻せば数字は一気に変わります。');
    }
    L.push('');
    if (T) {
      const n = T.next;
      L.push(`■ ${T.name || 'FF寺子屋'}に入ってください`);
      if (T.description) L.push(`　${T.description}`);
      L.push(`　次回 ${fmtDate(n.date)} ${n.time || ''}`.trimEnd());
      if (n.theme) L.push(`　テーマ：${n.theme}`);
      if (n.teacher) L.push(`　講師：${n.teacher}`);
      const link = n.url || (T.howToJoin || '').match(/https?:\/\/\S+/)?.[0];
      if (link) L.push(`　参加URL：${link}`);
    } else {
      L.push('■ FF寺子屋に入ってください（日程は追ってご案内します）');
    }
    L.push('');
    L.push('稼働は続いています。動けているなら、戻せます。');

    return {
      name, company: r['所属会社'] || '', role, cause,
      down, other, improved, drop, workDays, lastIn,
      needsReview, excluded,
      send: !excluded && !needsReview,     // 実際に送ってよいか（送信側はこれだけ見ればよい）
      excludeReason: excluded ? ((T && T.excludeReason) || '送信対象外に指定') : null,
      reviewReason: needsReview ? `8月の出退勤記録が0日（最終出勤：${lastIn || 'なし'}）。休職・離脱の可能性があるため要確認` : null,
      message: L.join('\n'),
    };
  });

  return {
    ok: true,
    count: targets.length,
    needsReview: targets.filter(t => t.needsReview).length,
    excluded: targets.filter(t => t.excluded).length,
    sendCount: targets.filter(t => t.send).length,
    holdCount: targets.filter(t => !t.send).length,
    scheduleReady: !!T,
    targets,
  };
}

/** 全体向けの開催案内（成績には一切触れない一般告知）。
    指導対象への個別DMとは別物：こちらは「参加のご案内」なので誰に出しても角が立たない。 */
export function buildAnnouncement() {
  const T = schedule();
  if (!T) return { ok: false, error: '開催情報が未設定です' };
  const n = T.next;
  const L = [];
  L.push(`【${T.name || 'FF寺子屋'} 開催のご案内】`);
  L.push('');
  L.push('Ruminaです。営業活動の一助となる社内勉強会が始まります。');
  L.push('アポイント獲得を伸ばしたい方は、ぜひご参加ください。');
  L.push('');
  L.push(`■ 初回 ${fmtDate(n.date)} ${n.time || ''}`.trimEnd());
  if (n.theme) L.push(`　テーマ：${n.theme}`);
  if (n.teacher) L.push(`　講師：${n.teacher}`);
  L.push('');
  const link = n.url || (T.howToJoin || '').match(/https?:\/\/\S+/)?.[0];
  if (link) { L.push('■ 参加URL（オンライン）'); L.push(`　${link}`); L.push(''); }
  if (T.cadence) { L.push(`■ 開催頻度：${T.cadence}`); L.push(''); }
  if (T.note) L.push(T.note);
  L.push('お気軽にご参加ください。お待ちしています。');
  return { ok: true, message: L.join('\n'), session: n, name: T.name || 'FF寺子屋' };
}

/* ============================================================
   アポインター全員への個別案内（会社方針：氏名・成績・平均差を必ず明記）
   ※ _RULE.md の「本人へ平均以下通知をしない」は、経営判断により本用途では
     適用しない旨の指示を受けている（2026-08-18）。ルール本文も要更新。
   本人へのDMのみ。他人の成績は一切含めない（自分の数字と全体平均だけ）。
   ============================================================ */
import * as cyzen from './cyzen.mjs';

export function buildAppointerInvites(opts = {}) {
  if (!cyzen.ready()) return { ok: false, error: 'cyzenのデータが未取込です' };
  const T = schedule();
  const all = (cyzen.roster().rows || []).filter(r => /アポインター/.test(r.attr || ''));
  const act = all.filter(r => r.days > 0);
  if (!act.length) return { ok: false, error: '期間内に稼働記録のあるアポインターがいません' };

  // 示す数字は「トップの実績」だけ（部門方針 2026-08-18）。
  // 本人の成績は計上漏れが多く確度が担保できないため、DMには載せない。
  const top = act.slice().sort((a, z) => z.apo - a.apo)[0];
  const topPerDay = top.days ? +(top.apo / top.days).toFixed(1) : 0;
  // 平均差は「訪問数」で出す。アポは計上率が低く(83名中20名)確度が担保できないため、
  // 本人へ差を突きつける数字には使わない（部門方針 2026-08-18 の③）。
  const avgVpd = +(act.reduce((n, r) => n + r.vpd, 0) / act.length).toFixed(1);
  const topVisitor = act.slice().sort((a, z) => z.vpd - a.vpd)[0];
  const excludeList = ((T && T.excludeNames) || []).map(normName);

  const targets = act.map(r => {
    const excluded = excludeList.includes(normName(r.name));
    const L = [];
    L.push(`${r.name} さん`);
    L.push('');
    L.push('Ruminaです。今月の調子はどうですか？');
    L.push('');
    const diff = +(avgVpd - r.vpd).toFixed(1);   // 正なら平均より少ない
    L.push('■ あなたの行動量（直近）');
    L.push(`　訪問 ${r.vpd}件/日（${r.days}日稼働・計${r.visits}件）`);
    L.push(diff > 0
      ? `　全体平均 ${avgVpd}件/日 との差：${diff}件/日 少ない`
      : `　全体平均 ${avgVpd}件/日 との差：${Math.abs(diff)}件/日 多い`);
    L.push('');
    L.push('■ 今月のトップ');
    L.push(`　アポ：1日平均 ${topPerDay}アポ（${top.name}さん）`);
    L.push(`　行動量：1日 ${topVisitor.vpd}件（${topVisitor.name}さん）`);
    L.push('　ぜひ一緒に追いついていきましょう！');
    L.push('');
    if (T) {
      const n = T.next;
      L.push(`■ ${T.name || 'FF寺子屋'}で型を持ち帰れます`);
      L.push(`　${fmtDate(n.date)} ${n.time || ''}`.trimEnd());
      if (n.theme) L.push(`　テーマ：${n.theme}`);
      if (n.teacher) L.push(`　講師：${n.teacher}`);
      const link = n.url || (T.howToJoin || '').match(/https?:\/\/\S+/)?.[0];
      if (link) L.push(`　参加URL：${link}`);
      if (T.cadence) L.push(`　（${T.cadence}）`);
      L.push('');
    }
    L.push('数字は必ずついてきます。一緒に追っていきましょう。');
    return { name: r.name, vpd: r.vpd, visits: r.visits, days: r.days, apo: r.apo,
      diffVsAvg: +(avgVpd - r.vpd).toFixed(1), excluded, send: !excluded, message: L.join('\n') };
  });

  return {
    ok: true,
    top: { name: top.name, apo: top.apo, days: top.days, perDay: topPerDay },
    topVisitor: { name: topVisitor.name, vpd: topVisitor.vpd },
    avgVpd,
    total: targets.length,
    sendCount: targets.filter(t => t.send).length,
    scheduleReady: !!T,
    targets,
  };
}
