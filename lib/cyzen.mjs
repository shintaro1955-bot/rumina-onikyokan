...
// 「2026/09/07 17:32」「2026-09-07T17:32:00」などから時（0-23）だけ取り出す。取れなければ null
export function hourOf(v) { const m = String(v || '').match(/(?:^|[ T])(\d{1,2}):(\d{2})/); if (!m) return null; const h = +m[1]; return (h >= 0 && h <= 23) ? h : null; }
  ...
  if (existsSync(repDir)) {
    for (const fn of readdirSync(repDir).filter(f => f.toLowerCase().endsWith('.csv'))) {
      for (const r of toObjects(readText(join(repDir, fn), true))) {
        ...
        else if (name.includes('アポ獲得')) { d.apo++; const h = hourOf(r['報告日時'] || r['訪問日時']); if (h != null) (d.apoAt || (d.apoAt = [])).push(h); }
...
// ライブ取り込み（cyzen API）で集計を差し替える。安全弁は呼び出し側で担保する。
export function applyLive(users, day, meta) {
  DB = { users, day, meta: { ...meta, source: 'api' } };
  return DB;
}
export function currentSource() { const d = db(); return d ? (d.meta.source || 'csv') : 'none'; }
...
/* ym（'YYYY-MM'）を渡すとその月の日次だけで集計する（GRAND PRIXの「今月」に合わせるため）。
   省略時は取り込んである全期間（直近31日）＝従来どおり。 */
export function roster({ ym = '' } = {}) {
  const d = db(); if (!d) return { ready: false, rows: [], summary: {} };
  const per = new Map();
  const dates = new Set();
  for (const rec of d.day.values()) {
    if (ym && String(rec.date || '').slice(0, 7) !== ym) continue;   // 月指定：その月の日次だけ
    dates.add(rec.date);
    const active = rec.visitsSelf > 0 || rec.apo > 0 || rec.workStart || rec.workEnd;
    if (!active) continue;
    let p = per.get(rec.code);
    if (!p) { p = { code: rec.code, days: 0, visits: 0, apo: 0, seiyaku: 0, haisen: 0, shodan: 0, hours: [], stamps: 0 }; per.set(rec.code, p); }
    p.days++; p.visits += rec.visitsSelf; p.apo += rec.apo; p.seiyaku += rec.seiyaku; p.haisen += rec.haisen; p.shodan += (rec.shodan || 0); p.stamps += rec.visitStamp;
  ...
  for (const [code, u] of d.users) {
    ...
    // 断られ数（アポインター＝訪問のうちアポにならなかった数の近似）。負にはしない。
    const rejects = Math.max(0, visits - apo);
    rows.push({ code, name: u.name, attr: u.attr, group: u.group, isCloser, days, visits, vpd, apo, apoRate, seiyaku: p ? p.seiyaku : 0, haisen: p ? p.haisen : 0, shodan: p ? p.shodan : 0, rejects, closeRate, hours, stamps: p ? p.stamps : 0, seg, why });
  }
  ...
  return { ready: true, rows, summary, bench: BENCH, ym: ym || null };
}

/* ---- 伸びているスタッフ：直近◯日 vs その前◯日 で行動量の伸びを測る ----
   「今これぐらいオペっている（＝到達水準）」＋「先週比の伸び」を返す。
   基準日はデータの最新日。recentDays日ずつ2窓に割って比較する。 */
export function trends({ recentDays = 7 } = {}) {
  const d = db(); if (!d) return { ready: false, rows: [] };
  const latest = latestDate(); if (!latest) return { ready: false, rows: [] };
  const dayMs = 86400000, end = Date.parse(latest + 'T00:00:00Z');
  const recFrom = end - (recentDays - 1) * dayMs;       // 直近窓の開始
  const priFrom = end - (recentDays * 2 - 1) * dayMs;   // その前の窓の開始
  const inRange = (ymd, from, to) => { const t = Date.parse(ymd + 'T00:00:00Z'); return t >= from && t <= to; };
  const per = new Map();
  const G = code => { let p = per.get(code); if (!p) { p = { rd: 0, rv: 0, ra: 0, pd: 0, pv: 0, pa: 0 }; per.set(code, p); } return p; };
  for (const rec of d.day.values()) {
    const active = rec.visitsSelf > 0 || rec.apo > 0 || rec.workStart || rec.workEnd;
    if (!active) continue;
    if (inRange(rec.date, recFrom, end)) { const p = G(rec.code); p.rd++; p.rv += rec.visitsSelf; p.ra += rec.apo; }
    else if (inRange(rec.date, priFrom, recFrom - dayMs)) { const p = G(rec.code); p.pd++; p.pv += rec.visitsSelf; p.pa += rec.apo; }
  }
  const rows = [];
  for (const [code, p] of per) {
    if (p.rd < 2) continue;                               // 直近に2日以上稼働した人だけ
    const u = d.users.get(code) || {};
    const recVpd = +(p.rv / p.rd).toFixed(1);
    const priVpd = p.pd ? +(p.pv / p.pd).toFixed(1) : 0;
    const deltaVpd = +(recVpd - priVpd).toFixed(1);
    const growth = priVpd > 0 ? Math.round((recVpd / priVpd - 1) * 100) : (recVpd > 0 ? null : 0); // null=新規/前週ゼロ
    const recApoRate = p.rv ? +(p.ra / p.rv * 100).toFixed(1) : 0;
    const priApoRate = p.pv ? +(p.pa / p.pv * 100).toFixed(1) : 0;
    rows.push({
      code, name: u.name, attr: u.attr,
      recDays: p.rd, recVisits: p.rv, recApo: p.ra, recVpd, recApoRate,
      priDays: p.pd, priVisits: p.pv, priApo: p.pa,   // 前週の実数（底上げ一覧の先週比で使う）
      priVpd, priApoRate, deltaVpd, growth,
      apoDelta: p.ra - p.pa,
    });
  }
  // 伸び順（訪問/日の増分）。前週ゼロからの新規は下駄を履かせず deltaVpd で評価。
  rows.sort((a, z) => z.deltaVpd - a.deltaVpd || z.recVpd - a.recVpd);
  return { ready: true, recentDays, anchor: latest, rows };
}
...
// 連続稼働日数（最新の稼働日から遡って途切れないアクティブ日数）。継続性/ストリーク用。
export function personStreak(code, minVisits = 1) {
  const d = db(); if (!d || !code) return 0;
  const days = [];
  for (const rec of d.day.values()) {
    if (rec.code !== code) continue;
    const active = rec.visitsSelf >= minVisits || rec.apo > 0 || rec.workStart || rec.workEnd;
    if (active) days.push(rec.date);
  }
  if (!days.length) return 0;
  days.sort((a, b) => b.localeCompare(a));   // 新しい順
  let streak = 1;
  for (let i = 1; i < days.length; i++) {
    const prev = Date.parse(days[i - 1] + 'T00:00:00Z'), cur = Date.parse(days[i] + 'T00:00:00Z');
    const gap = Math.round((prev - cur) / 86400000);
    if (gap === 1) streak++;
    else if (gap === 0) continue;   // 同日重複は無視
    else break;
  }
  return streak;
}

// 本人の最新稼働日1件（Today Hero用）。稼働のある最も新しい日を返す。
export function personToday(code) {
  const d = db(); if (!d || !code) return null;
  let best = null;
  for (const rec of d.day.values()) {
    if (rec.code !== code) continue;
  ...
  if (!best) return null;
  return { date: best.date, visits: best.visitsSelf, apo: best.apo, seiyaku: best.seiyaku, workStart: best.workStart, workEnd: best.workEnd };
}
...
export function personSummary(code) {
  ...
  return {
    ...
    reportRate: days ? +(reported / days).toFixed(2) : 0,   // 報告品質＝勤務終了報告を出した日の割合
    periodDays: dates.size, empty: false,
  };
}


// ===== 1日の実績（ポータルの朝ミッション／夕方の追い込み用） =====
// その日に稼働のあった人だけ返す（visits/apo/shodan/seiyaku と出勤・勤務終了の有無）。
export function dayFacts(date) {
  const d = db(); if (!d || !date) return [];
  const out = [];
  for (const rec of d.day.values()) {
    if (rec.date !== date) continue;
    const u = d.users.get(rec.code) || {};
    if (u.suspended) continue;
    const active = rec.visitsSelf > 0 || rec.apo > 0 || rec.seiyaku > 0 || rec.shodan > 0 || rec.workStart || rec.workEnd;
    if (!active) continue;
    out.push({ code: rec.code, name: u.name || '', visits: rec.visitsSelf || 0, apo: rec.apo || 0, shodan: rec.shodan || 0, seiyaku: rec.seiyaku || 0, started: !!rec.workStart, ended: !!rec.workEnd });
  }
  return out.sort((a, z) => z.apo - a.apo || z.visits - a.visits);
}

// ===== アポ報告の時間帯（直近N日・全員） =====
// 報告書の「報告日時」の時刻ベース。アポを取った瞬間と報告時刻はずれることがあるので「報告時刻」と明示して使う。
export function apoHours(days = 30) {
  const d = db(); if (!d) return { total: 0, hist: new Array(24).fill(0), days };
  const from = new Date(Date.now() + 9 * 3600 * 1000 - Math.max(1, days) * 86400000).toISOString().slice(0, 10);
  const hist = new Array(24).fill(0); let total = 0;
  for (const rec of d.day.values()) {
    if (!rec.date || rec.date < from || !Array.isArray(rec.apoAt)) continue;
    for (const h of rec.apoAt) { if (h >= 0 && h <= 23) { hist[h]++; total++; } }
  }
  return { total, hist, days, from };
}

// ===== 本人の1か月（ポータルのマイページ：セクター・スティント用） =====
// セクター＝アポ獲得報告の「報告時刻」を 朝〜13時／13〜17時／17時〜 の3区間に分けた件数。
// 取った瞬間ではなく報告した時刻なので、画面でも「報告時刻」と明示して使うこと。
// best＝その月の全員の中での区間ごとの最多件数（名前は返さない）。prev＝本人の前月。
export function driverMonth(name, ym) {
  const d = db(); if (!d || !name || !/^\d{4}-\d{2}$/.test(String(ym || ''))) return null;
  const nz = (x) => String(x == null ? '' : x).normalize('NFKC').replace(/[\s\u3000]/g, '');
  const target = nz(name);
  let code = null, uname = '';
  for (const [c, u] of d.users) { if (u && !u.suspended && nz(u.name) === target) { code = c; uname = u.name; break; } }
  if (!code) return null;
  const secOf = (h) => (h < 13 ? 0 : (h < 17 ? 1 : 2));
  const [y, m] = ym.split('-').map(Number); const pd = new Date(Date.UTC(y, m - 2, 1)); const pym = pd.toISOString().slice(0, 7);
  const mine = [0, 0, 0], prev = [0, 0, 0], all = new Map(), days = [];
  for (const rec of d.day.values()) {
    if (!rec || !rec.date) continue;
    const inM = rec.date.startsWith(ym), inP = rec.date.startsWith(pym);
    if (!inM && !inP) continue;
    const u = d.users.get(rec.code) || {}; if (u.suspended) continue;
    const hrs = Array.isArray(rec.apoAt) ? rec.apoAt.filter((h) => h >= 0 && h <= 23) : [];
    if (inM && hrs.length) { const a = all.get(rec.code) || all.set(rec.code, [0, 0, 0]).get(rec.code); for (const h of hrs) a[secOf(h)]++; }
    if (rec.code !== code) continue;
    if (inP) { for (const h of hrs) prev[secOf(h)]++; continue; }
    for (const h of hrs) mine[secOf(h)]++;
    const active = rec.visitsSelf > 0 || rec.apo > 0 || rec.seiyaku > 0 || rec.shodan > 0 || !!rec.workStart || !!rec.workEnd;
    days.push({ date: rec.date, active: !!active, visits: rec.visitsSelf || 0, apo: rec.apo || 0 });
  }
  const best = [0, 0, 0]; for (const a of all.values()) for (let i = 0; i < 3; i++) if (a[i] > best[i]) best[i] = a[i];
  days.sort((a, z) => a.date.localeCompare(z.date));
  return { name: uname, ym, days, sectors: mine, prevSectors: prev, bestSectors: best, activeDays: days.filter((x) => x.active).length };
}

// ===== 連続アポ記録（稼働した日を数える。休みの日は飛ばし、稼働してアポ0の日で途切れる） =====
export function apoStreaks() {
  const d = db(); if (!d) return [];
  const byCode = new Map();
  for (const rec of d.day.values()) {
    const active = rec.visitsSelf > 0 || rec.apo > 0 || rec.workStart || rec.workEnd;
    if (!active) continue;
    (byCode.get(rec.code) || byCode.set(rec.code, []).get(rec.code)).push(rec);
  }
  const out = [];
  for (const [code, recs] of byCode) {
    const u = d.users.get(code) || {}; if (u.suspended) continue;
    recs.sort((a, z) => z.date.localeCompare(a.date));      // 新しい順
    let cur = 0, lastDate = null, brokeAt = null;
    for (const r of recs) { if (r.apo > 0) { cur++; if (!lastDate) lastDate = r.date; } else { brokeAt = r.date; break; } }
    // 最長記録（古い順に走査）
    let best = 0, run = 0; for (const r of recs.slice().reverse()) { if (r.apo > 0) { run++; if (run > best) best = run; } else run = 0; }
    if (cur > 0 || best > 0) out.push({ code, name: u.name || '', streak: cur, lastDate, brokeAt, best });
  }
  return out.sort((a, z) => z.streak - a.streak || z.best - a.best || String(a.name).localeCompare(String(z.name)));
}
