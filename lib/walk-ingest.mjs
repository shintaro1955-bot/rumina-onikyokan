...
/** 1日ぶん（JST・'YYYY-MM-DD'）の歩行距離。夜の通知で「今日歩いた距離」を出すために使う。 */
export function dayStats(date, visitsByCode = null) {
  const db = load();
  const all = Object.values(db.days || {}).filter((r) => r.date === date && (r.walkM > 0 || r.rideM > 0));
  const rows = all.map((r) => {
    const u = (db.users || {})[r.uid] || {};
    const walkKm = +(r.walkM / 1000).toFixed(1);
    const v = visitsByCode && u.code ? visitsByCode.get(u.code) : null;
    return { code: u.code || r.uid, name: u.name || u.code || r.uid, group: u.group || '',
             walkKm, rideKm: +(r.rideM / 1000).toFixed(1), points: r.points, visits: v ? v.visits : null };
  }).sort((a, z) => z.walkKm - a.walkKm);
  return { ready: rows.length > 0, date, count: rows.length,
           totalWalkKm: +(rows.reduce((n, r) => n + r.walkKm, 0)).toFixed(1), updatedAt: db.updatedAt || null, rows };
}
