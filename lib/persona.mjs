 ...
 */
export async function customerReply({ ctype = '警戒', persona, difficulty, product, history = [], salesText = '', drill = '' } = {}) {
  if (!API_KEY) return null;
  // 3軸のどれかが来ていれば新（avatar）、無ければ旧（ctype）で組み立てる。
  const { msgs, sys } = buildInput({ ctype, persona, difficulty, product, history, salesText, drill });
  try {
...
function buildInput({ ctype, persona, difficulty, product, history = [], salesText = '', drill = '' }) {
  const msgs = [];
  ...
  let sys = (persona || difficulty || product) ? systemPromptV2({ persona, difficulty, product }) : systemPrompt(ctype);
  // 課題ロープレ：その日の弱点を必ず突いてくる客にする（鬼教官の日次コーチから渡ってくる）。
  if (drill) sys += '\n\n【この練習での追加の役づくり】\n' + String(drill).slice(0, 400) + '\nこの縛りは最優先。ただしセリフだけを話し、説明や解説はしない。';
  return { msgs, sys };
}
