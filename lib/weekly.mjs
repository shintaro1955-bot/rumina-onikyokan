...
// 誤差を「増えた/減った」と言うと指導が信用されない。50件回る人の1.5件差(3%)は誤差、
// 16件の人の1.5件差(9%)は動きなので、幅は母数に連動させる（最低1.5件・または先週比10%）。
const deadband = (base) => Math.max(1.5, base * 0.1);
function shape(t, bench) {
  const d = +(t.recVpd - t.priVpd).toFixed(1);
  const dir = Math.abs(d) < deadband(t.priVpd) ? 'flat' : (d > 0 ? 'up' : 'down');
  // 自動送信しない（人が確認する）：
  //  ・稼働日数が半分以下まで落ちて、今週2日以下
  //  ・今週の訪問が0件（止まっているので、励ましを自動で送る場面ではない）
  const atRisk = t.priDays > 0 && ((t.recDays <= 2 && t.recDays <= t.priDays * 0.5) || t.recVpd === 0);
...
export function messageFor(m) {
  ...
  if (m.dir === 'up') {
    ...
    // 中央値を出すのは、その人が中央値を下回っている時だけ。
    // 中央値の3倍出している人に中央値を示しても意味がない。
    if (m.median != null && m.recVpd < m.median) L.push(`チームの中央値は${m.median}件です。`);
