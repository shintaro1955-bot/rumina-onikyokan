...
function talkFidelitySection(a) {
  ...
  const baseline = model
    ...
    : `基準値：<span class="text-neutral-600 font-medium">初期値</span>（成功モデル未登録 — 成功モデルの録音を登録すると実データ基準に）`;
  ...
  return section('成功モデルとの乖離カルテ',
    inner,
    '成功モデルの勝ちの型を"基準値"に、あなたのトークを健康診断。どの型がどれだけ足りていないかをカルテで可視化。');
}
...
/* ---------- ④ レポート ---------- */
/* ロープレチェック（説明力・差別化）— 研修と同じ評価軸を鬼教官に適用 */
function roleplayCheckSection(a, b) {
  ...
  return section('ロープレチェック（説明力・差別化）', body, '研修と同じ観点。発火機会／めんどくさいスイッチ／差別化の説明力で、明日から鬼教官が日々チェックする。');
}
...
async function loadBuildup() {
  ...
  const PER = 12;
  const section = (k) => {
    const list = d.rows.filter(r => r.rung === k);
    ...
    return `<div class="mt-4">
      ...
      ${list.length > PER ? `<div class="text-[11px] text-neutral-400 mt-1">ほか${list.length - PER}人（訪問数の多い順に${PER}人まで表示）</div>` : ''}
    </div>`;
  };
  const sections = ['記録', '稼働', '行動量', 'トーク', 'クロージング'].map(section).join('');
...
function deviateNarrate(a) {
  ...
  if (!behind.length) { out.push({ tone: 'good', title: '結論', text: '成功モデルとほぼ同じ型で動けている。この水準を維持しろ。' }); return out; }
  const t = behind[0], s = behind[1];
  out.push({ tone: 'harsh', title: '結論', text: `君は成功モデルと${behind.length}項目で乖離している。最大は「${t.k}」——君${t.me}${t.u}に対しモデルは${t.top}${t.u}。ここが全ての起点だ。` });
  if (s) out.push({ tone: 'warn', title: '次の乖離', text: `次が「${s.k}」（君${s.me}${s.u} / モデル${s.top}${s.u}）。ここもモデルとの差が大きい。` });
  out.push({ tone: 'close', title: 'やること', text: `全部を一度に真似るな。まず「${t.k}」だけを成功モデルの水準に寄せろ。1項目でいい。それが最短で差を詰める。` });
  return out;
}
function deviationSection(a) {
  ...
  const rows = items.map(x => {
    ...
    return `<div class="flex items-center gap-3 py-1.5">
      ...
      <div class="flex-1 text-xs text-neutral-500 tabular-nums">君 ${x.me}${x.u} / モデル ${x.top}${x.u}</div>
...
async function loadTodayFocus() {
  ...
  const cell = (label, mineV, topV, teamV, unit, shortV, note) => {
    if (topV == null && teamV == null) return '';
    const target = topV;                     // 目標を置かない軸(距離)は topV=null で「あるべき姿」を出さない
    const has = mineV != null;
    const ok = has && target != null && shortV === 0;
    return `<div style="flex:1;min-width:130px;padding:12px 14px;border:1px solid ${ok ? 'var(--primary)' : 'var(--border)'};border-radius:14px;background:var(--surface);text-align:left">
      ...
      ${target != null ? `<div style="font-size:11.5px;margin-top:2px;color:#b45309;font-weight:700">あるべき姿 ${target}${unit}</div>` : ''}
      ${teamV != null ? `<div class="muted" style="font-size:10.5px">みんなの平均 ${teamV}${unit}</div>` : ''}
      <div style="font-size:12.5px;font-weight:700;margin-top:4px;color:${target == null ? 'var(--muted)' : !has ? 'var(--muted)' : ok ? 'var(--primary)' : '#e11d48'}">
        ${target == null ? (note || '参考値') : !has ? '記録待ち' : ok ? '届いています' : `あと ${shortV}${unit}`}</div>
    </div>`;
  };
  ...
  if (g && g.ready && g.team && g.team.people) {
    ...
    board = `<div style="max-width:680px;margin:16px auto 0">
      <div style="display:flex;gap:10px;flex-wrap:wrap">
        ...
        ${cell('歩いた距離', (g.found && me.walkKm != null) ? me.walkKm : null, gl.walkKm, t.walkKm, 'km', sh.walkKm, '目標は置かない')}
      </div>
      ...
      <div class="muted" style="font-size:11px;margin-top:8px">あるべき姿＝${g.best ? `${esc(g.best.name)}さんの1日（直近30日の平均）。量ではなく決める率で追う` : '上位2%の水準'} ・ 歩いた距離はGPSの記録が粗いため目標を置いていません ・ みんなの平均＝${esc(g.date)}に動いている${t.people}人</div>
    </div>`;
  }
...
async function loadToday() {
  ...
  const hero = `<div class="fo-card" style="padding:20px">
    ...
    ${today ? '' : `<div style="margin-top:16px"><span class="muted" style="font-size:13px">今日の記録はこれからです。回った分だけここに積み上がります。</span></div>`}
...
async function loadMePerf() {
  ...
  wrap.innerHTML = `${h1('My Performance')}<div id="portalCard" class="mb-4"></div><div id="weekMove" class="mb-4"></div>${moCard}${dayCard}${statCard}`
