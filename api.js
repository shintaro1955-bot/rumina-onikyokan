...
window.API = (function () {
  ...
  async function apoCoachView() { try { const r = await fetch('/api/cyzen/apo-coach-view'); return r.ok ? r.json() : { ok: false, error: '取得できませんでした' }; } catch { return { ok: false, error: '接続に失敗しました' }; } }
  ...
  async function lineUnlinkedDiag() {
    const r = await fetch('/api/line/unlinked-diag');
    const j = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(j.error || '未連携の内訳の取得に失敗しました');
    return j;
  }
  async function weeklyMine() {
    const r = await fetch('/api/weekly/mine');
    const j = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(j.error || '先週からの動きの取得に失敗しました');
    return j;
  }
  async function weeklyNotify(live) {
    const r = await fetch('/api/weekly/notify' + (live ? '?live=1' : ''), { method: 'POST' });
    const j = await r.json().catch(() => ({}));
  ...
  async function teamBench() {
    ...
    const j = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(j.error || 'チームの基準の取得に失敗しました');
    return j;
  }
  ...
  async function todayGap() {
    const r = await fetch('/api/today/gap');
    const j = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(j.error || '今日の指標の取得に失敗しました');
    return j;
  }
